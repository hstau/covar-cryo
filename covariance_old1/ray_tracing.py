...
def gtrace(sp, u1, v1, rad2, pp, unit, v_dim, p_dim, ind2, ind3, factor, mat):
    ...
    if ind > 0:            # if within mask
        ...
        # negative side
        pos.append(trace(rad2, pp, -unit, v_dim, 1,factor))
...
def trace(rad2, pp, unit, v_dim,start,factor):
    NX = v_dim[0] 
    NY = v_dim[1] 
    NZ = v_dim[2]
    N = NX*NY*NZ
...
def ind_we(pos,v_dim,ind3,factor):
    ...
    if pos.shape[0] > 0:
        ...
        # collapsing
        A=1.0/factor*coo_matrix((on,(in2,on)), shape=(N, 1))
        ...
        a, b, c = decomp3(in1, v_dim)  
        v_dim1 = v_dim
        # scaling and NO masking
        a = np.floor(a/2).reshape(a.shape[0],1)
        b = np.floor(b/2).reshape(b.shape[0],1)
        c = np.floor(c/2).reshape(c.shape[0],1)
        pos1 = np.hstack((a,b,c)) # a b c already shifted
        # scaling
        v_dim1 = np.floor(v_dim1/2)
        # recalculate A
        in2 = process(pos1,v_dim1,0) # no shift applied
        on = np.ones(in2.shape)
        A = coo_matrix((w,(in2,on)),shape=(N, 1))) #rsparse(in2,on,w,N,1) # collapsing
        in1 = A.row
        # aux = A.col
        w = A.data
        # storing results
...
def project(euler,v_dim,p_dim,ind2,ind3,i):
    factor = 20
    ...
    M = np.amax(np.hstack((find2,cind2)))
    N = np.amax(np.hstack((find3,cind3)))
    # crate a sparse matrix holding mat
    row = mat[:,0]  
    col = mat[:,1]
    w = mat[:,2]
    # factor 10 reduction; use of csr matrix
    mat = 1./10*csr_matrix((w,(row,col)),shape=(M, N)))
    return mat
