function mask = create_mask(fraction, NX,varargin)  % percent < 1
...
i = 1;
last = i-1;
while (w(i) > fraction*w(1))
    i = i+1;
    last = i;
end
% fraction of non-zeros
ratio = last/size(im(:),1)
% binary mask
mask = zeros(NX,NX,NX);
...
mask = mask > 0;
%output
out_mask = strcat('sfvol_var_perc',num2str(100*fraction,'%03d'),'_mask',ext);
writeSPIDERfile(out_mask,mask);
% now the remaining
remain = zeros(NX,NX,NX);
remain(I(last+1:end)) = im(I(last+1:end));
out_remain = strcat('sfvol_var_perc',num2str(100*fraction,'%03d'),'_remain',ext);
