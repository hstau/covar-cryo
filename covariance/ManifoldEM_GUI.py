...
################################################################################
# MAC nonlethal error messages (bash):

    #1. error upon closing any 'warning box': " modalSession has been exited prematurely...
        #... - check for a reentrant call to endModalSession: "
        ### comments: nonlethal, mac-specific Qt4 error (ignore)
    #2. error on Browse -> Cancel: " IOError: [Errno 2] No such file or directory: u'' "
        ### comments: nonlethal (ignore)
    #3. error on 2D Class Avg: "MatplotlibDeprecationWarning: Adding an axes using the same...
...
class Mayavi_S2(HasTraits): # S2 Orientation Sphere, Electrostatic Potential Map
    ...
    def update_scene1(self):
        ...
        splot.actor.actor.scale = np.multiply(self.S2_scale,
                                    np.array([P1.S2rescale*len(P1.df_vol)/float(np.sqrt(2)),
                                              P1.S2rescale*len(P1.df_vol)/float(np.sqrt(2)),
                                              P1.S2rescale*len(P1.df_vol)/float(np.sqrt(2))]))
...
class Mayavi_Rho(HasTraits): #electrostatic potential map, P4
    ...
    #S2_scale_all = List([0.0,.2,.4,.6,.8,1.0,1.2,1.4,1.6,1.8,2.0])
    #S2_scale = Enum(1.0, values='S2_scale_all')
    S2_scale = Float
    anchorsUpdate = []
    trashUpdate = []
    ...
    def update_scene3(self):        
        ...
        if P3.dictGen == False: #if P3 dictionaries not yet created (happens once on P3)
            ...
            cplot = mlab.contour3d(mirror,contours=self.isosurface,
                                   color=(0.5, 0.5, 0.5),
                                   figure=Mayavi_Rho.fig3)
            cplot.actor.actor.orientation = np.array([0., -90., 0.]) #zoozoo?
        ...
        def release_callback(vtk_obj, event): #left mouse release callback
            if self.click_on == 1:
                ...
                self.scene3.mlab.view(azimuth=float(phis[indexes[-1]]),
                                      ...
                                      distance=current[2],
                                      roll=float(phis[indexes[-1]])+270, #zoozoo
                                      #reset_roll=False,
                                      figure=Mayavi_Rho.fig3)
    ...
    view = View(VGroup(
                ...
                VGroup(
                    HGroup(
                        ...
                        #Item('S2_scale',springy=True,show_label=True,
                        #tooltip='Change the relative scale of the S2 sphere with respect to the volume map above.'),
                        #Item('volume_alpha',springy=True,show_label=True,
                        #tooltip='Change the opacity of the volume map above.'),
                        #Item('isosurface',springy=True,show_label=True,
                        #tooltip='Change the isosurface level of the volume map above.'),
                    ),
...
################################################################################
# mayaVI viz 3 (3D Energy Landscape):

class Mayavi_LS(HasTraits):
    scene4 = Instance(MlabSceneModel, ())

    EL_plot = Instance(PipelineBase)
    cursor2d = Instance(PipelineBase)
    pointsPlot = Instance(PipelineBase)
    edgesPlot1 = Instance(PipelineBase)
    edgesPlot2 = Instance(PipelineBase)

    path_width = Enum(1,3,5)
    reset_points = Button('   Reset Points   ')
    units = Str
    pixelsSqr = Str
    energy_path = Button('2D Energy Path')
    click_counter = Int
    custom_title = Str
    custom_path_integral = Float
    custom_save = Button('Save Custom Path')
    action_title = Str
    start = Button('Confirm Start Point')
    end = Button('Confirm End Point')
    action_compute = Button('Compute Path of Least Action')
    action_save = Button('Save LA Path')
    least_action_path_integral = Float
    least_action_output = Int
    reset_view = Button('Reset View')
    user_final = Button('Generate Custom 3D Sequence')
    LA_final = Button('Generate LA 3D Sequence')
    ...
    yList = []
    zList = []
    allBlocks1 = [] #every point across line paths
    allBlocks1u = [] #unique of above list
    allBlocks3 = [] #every point and accompanying blocks across line paths
    allBlocks3u = [] #unique of above list
    allBlocks5 = [] #every point and accompanying blocks across line paths
    allBlocks5u = [] #unique of above list
    connections = []

    # temporary fix:
    LS2d = np.random.uniform(0, 2, (70, 70)) #still needs to read in actual data

    #fnameOM2d = os.path.join(P1.user_directory, 'outputs_{}/ELConc50/OM/S2_OM2d'.format(p.proj_name))  # occupancy
    #fnameEL2d = os.path.join(P1.user_directory, 'outputs_{}/ELConc50/OM/S2_EL2d'.format(p.proj_name))  # energy

    #Occ2d = np.fromfile(fnameOM2d, dtype=int) #2d occ file (npy)
    #LS2d = np.fromfile(fnameEL2d) #2d energy file from above (npy)
    #LS2d = np.rot90(LS2d, 1) #OPTIONAL
    '''
    fnameLS = os.path.join(inputDir,'EL_test/EL_Ribo_2d.txt')
    try:
        LS2d = loadtxt(fnameLS,float,delimiter=',')
    except ValueError:
        LS2d = loadtxt(fnameLS,float)
    #LS2d = np.rot90(LS2d,1)
    ''' 

    def _click_counter_default(self):
        return 0

    def _custom_path_integral_default(self):
        return 0

    def _units_default(self):
        return 'kcal/mol'

    def _pixelsSqr_default(self):
        return 'pixels%s' % (u"\u00B2")

    def _custom_title_default(self):
        return 'Custom Path Analysis'

    def _action_title_default(self):
        return 'Least Action Path Analysis'

    def _least_action_path_integral_default(self):
        return 0

    def _least_action_output_default(self):
        return 0

    @on_trait_change('custom_save')
    def customSave(self):
        if len(self.allBlocks1) > 0:
            customSaveDialog()
            if P6.custom_save == QtGui.QMessageBox.Yes:
                # create directory for text file:                
                timestr = time.strftime("%H%M%S")
                outDirPath = os.path.join(P1.user_directory, 'outputs_{}/path_analysis'.format(p.proj_name))
                try:
                ...
                # send paths to text file:
                if self.path_width == 1:
                    coord1 = [] #temp list for coords, width 1
                    erg1 = [] #temp list for energies, width 1
                    # remove overlaps from width 1 list:
                    for i in np.vstack(self.allBlocks1): 
                        j,k = i
                        if [j,k] not in self.allBlocks1u:
                            coord1.append([j,k])
                            erg1.append(self.LS2d[j,k])
                    completeName = os.path.join(outDirPath, 'custom_path_w1_%s.txt' % (timestr))
                    p.selPts_2D = np.vstack(coord1)
                    coord_erg1 = np.concatenate((np.vstack(coord1),np.vstack(erg1)),axis=1)
                    np.savetxt(completeName,coord_erg1,delimiter='\t',fmt='%1.0f %1.0f %1.3f')

                elif self.path_width == 3:
                    coord3 = [] #temp list for coords, width 3
                    erg3 = [] #temp list for energies, width 3
                    # remove overlaps from width 3 list:
                    for i in np.vstack(self.allBlocks3): 
                        j,k = i
                        if [j,k] not in self.allBlocks3u:
                            coord3.append([j,k])
                            erg3.append(self.LS2d[j,k])
                    completeName = os.path.join(outDirPath, 'custom_path_w3_%s.txt' % (timestr))
                    p.selPts_2D = np.vstack(coord3)
                    coord_erg3 = np.concatenate((np.vstack(coord3),np.vstack(erg3)),axis=1)
                    np.savetxt(completeName,coord_erg3,delimiter='\t',fmt='%1.0f %1.0f %1.3f')

                elif self.path_width == 5:
                    coord5 = [] #temp list for coords, width 5
                    erg5 = [] #temp list for energies, width 5
                    # remove overlaps from width 5 list:
                    for i in np.vstack(self.allBlocks5):
                        j,k = i
                        if [j,k] not in self.allBlocks5u:
                            coord5.append([j,k])
                            erg5.append(self.LS2d[j,k])
                    completeName = os.path.join(outDirPath, 'custom_path_w5_%s.txt' % (timestr))
                    p.selPts_2D = np.vstack(coord5)
                    coord_erg5 = np.concatenate((np.vstack(coord5),np.vstack(erg5)),axis=1)
                    np.savetxt(completeName,coord_erg5,delimiter='\t',fmt='%1.0f %1.0f %1.3f')
                    
# least action menu:
    @on_trait_change('action_compute')
    def action_dialog(self):
        if len(self.history) == 2:
            leastActionDialog()
            if P6.action_start == QtGui.QMessageBox.Yes:
                self.least_action_output = 1
                print('start point:', (self.history[0])[0]+(np.shape(self.LS2d)[0]/2), #x0
                                      (self.history[0])[1]+(np.shape(self.LS2d)[0]/2)) #y0                                
                print('end point:', (self.history[1])[0]+(np.shape(self.LS2d)[0]/2), #x1
                                      (self.history[1])[1]+(np.shape(self.LS2d)[0]/2)) #y1 
            #POLARIS_BE.least_action() #send start and end points to external script
        else:
            leastActionError()

    @on_trait_change('action_save')
    def actionSave(self): #check if least action has been run yet (or via disabling buttons)
        if self.least_action_output == 0:
            actionNLSAError()
        else:
            actionSaveDialog()
            if P6.action_save == QtGui.QMessageBox.Yes: #send all points to external script
                # create directory for text file:                
                timestr = time.strftime("%H%M%S")
                outDirPath = os.path.join(P1.user_directory, 'outputs_{}/path_analysis'.format(p.proj_name))
                try:
                    ...
                    if e.errno != errno.EEXIST:
                        raise                   
                # send paths to text file:
                LA_output = np.zeros((10,3)) #change this with real output! zoozoo
                completeName = os.path.join(outDirPath, 'least_path_%s.txt' % (timestr))
                p.leastPts = np.vstack(LA_output) #change to points only (no energies)                   
                #coord_erg1 = np.concatenate((np.vstack(coord1),np.vstack(erg1)),axis=1)
                np.savetxt(completeName,LA_output,delimiter='\t',fmt='%1.0f %1.0f %1.3f')               

    @on_trait_change('user_final')
    def userFinal(self):
        userFinalDialog()
        if P6.custom_gen == QtGui.QMessageBox.Yes: #if final file complete
            completeDialog()

    @on_trait_change('LA_final')
    def laFinal(self):
        laFinalDialog()
        if P6.action_gen == QtGui.QMessageBox.Yes: #if final file complete
            completeDialog()
               
    @on_trait_change('reset_points')
    def reset_scene4(self):
        ...
        self.custom_path_integral = 0
        self.pointsPlot.mlab_source.reset(x=self.userPtsX,
                                        y=self.userPtsY,
                                        z=self.userPtsZ)
        self.edgesPlot1.mlab_source.reset()
        self.cursor2d.mlab_source.reset(x=0,y=0,z=0)
        self.cursor2d.actor.actor.property.opacity = 0.0
        EL_title.property.opacity = 1.0

        self.pointsPlot.mlab_source.reset()
        self.pointsPlot.mlab_source.dataset.lines = []

        self.edgesPlot2.mlab_source.reset()
        self.scene4.mlab.view(EL_view[0],EL_view[1],EL_view[2],focalpoint='auto', figure=Mayavi_LS.fig4)

    @on_trait_change('reset_view')
    def update_view(self):
        self.scene5.mlab.view(-45, 55, figure=Mayavi_LS.fig5)
        self.scene6.mlab.view(-45, 55, figure=Mayavi_LS.fig6)

    @on_trait_change('scene4.activated')
    def update_scene4(self):
        global EL_title, EL_view
        Mayavi_LS.fig4 = mlab.figure(4, bgcolor=(.5,.5,.5))
        self.scene4.mlab.clf(figure=Mayavi_LS.fig4)
        
        self.EL_plot = self.scene4.mlab.imshow(self.LS2d, colormap='jet',
                                         figure=Mayavi_LS.fig4)

        cbar = mlab.scalarbar(title='Energy\n(kcal/mol)\n', orientation='vertical', \
                            nb_labels=3, label_fmt='%.1f')
        cbar.scalar_bar_representation.position = [0.9, 0.15]
        #self.EL_plot.module_manager.scalar_lut_manager.label_text_property.font_size = 8 #doesn't work
        self.EL_plot.module_manager.scalar_lut_manager.scalar_bar_widget.repositionable = False
        self.EL_plot.module_manager.scalar_lut_manager.scalar_bar_widget.resizable = False
        
        EL_title = self.scene4.mlab.title('Click on the landscape', size=0.3,
                               opacity=1.0, figure=Mayavi_LS.fig4)
        EL_title.property.vertical_justification='center' #or 'top'
        # scene views:
        EL_view = mlab.view(figure=Mayavi_LS.fig4)
        # update axes based on data dimensionality:
        self.scene4.mlab.axes(xlabel="CC1",
                              ylabel="CC2",
                              y_axis_visibility=False,
                              ranges = [0, len(self.LS2d), 0, len(self.LS2d[0]), 0, 0],
                              figure=Mayavi_LS.fig4)
        Mayavi_LS.fig4.scene.scene.interactor.interactor_style = \
                            tvtk.InteractorStyleImage() #pan/zoom only
        # main cursor (black cross):
        self.cursor2d = self.scene4.mlab.points3d(0,0,0,
                                             mode='2dcross',color=(0,0,0),
                                             resolution=50,opacity=0,
                                             scale_factor=(np.shape(self.LS2d)[0])/10,
                                             figure=Mayavi_LS.fig4)
        # user points (white crosses):
        self.pointsPlot = self.scene4.mlab.points3d(0,0,0,
                                           mode='2dcross',color=(1,1,1),
                                           resolution=50,opacity=0,
                                           scale_factor=(np.shape(self.LS2d)[0])/15,
                                           figure=Mayavi_LS.fig4)
        # connections 1 (fine point line, always present):
        self.edgesPlot1 = self.scene4.mlab.pipeline.tube(self.pointsPlot,tube_radius=.2)
        self.edgesPlot1.filter.radius_factor = .2
        self.edgesPlot1.filter.number_of_sides = 12
        self.edgesPlot1.filter.output_points_precision = 3
        self.scene4.mlab.pipeline.surface(self.edgesPlot1, color=(0,0,0),
                                          opacity=1)
        # connections 2 (advanced line, custom width):
        self.edgesPlot2 = self.scene4.mlab.pipeline.tube(self.pointsPlot,tube_radius=1)
        self.edgesPlot2.filter.radius_factor = 1.
        self.edgesPlot2.filter.number_of_sides = 12
        self.edgesPlot2.filter.output_points_precision = 3
        self.edgesPlot2.filter.vary_radius = 'vary_radius_by_scalar'
        self.scene4.mlab.pipeline.surface(self.edgesPlot2, color=(0,0,0),
                                          opacity=.25)
        # set picker:
        picker = Mayavi_LS.fig4.on_mouse_pick(self.picker2d_callback)
        picker.tolerance = 0.01

    def picker2d_callback(self, picker_obj):
        # track user data indices:
        EL_title.property.opacity = 0.0
        self.cursor2d.actor.actor.property.opacity = 1.0
        self.pointsPlot.actor.actor.property.opacity = 1.0
        point2d = picker_obj.mapper_position
        self.click_counter += 1
        x_, y_, z_ = point2d
        self.history.append([int(x_),int(y_),int(z_)])
        if self.custom_path_integral == 0: #append energy of first point to path integral
            temp_x0 = (self.history[0])[0]+(np.shape(self.LS2d)[0]/2)
            temp_y0 = (self.history[0])[1]+(np.shape(self.LS2d)[0]/2)
            self.custom_path_integral = self.LS2d[temp_x0,temp_y0]
        np.vstack(self.history)
        # view selected data indices:
        #x_T = x_+(np.shape(self.LS2d)[0]/2) # transform x points
        #y_T = y_+(np.shape(self.LS2d)[1]/2) # transform y points
        #print("Data indices: %i, %i = %i" % (x_T, y_T, self.LS2d[int(x_T),int(y_T)]))

        # add user point glyphs:
        if (len(self.history) > 1) and (self.history[-1] == self.history[-2]):
            self.click_counter -= 1 #don't count duplicate points
        else:
            self.cursor2d.mlab_source.reset(x=x_,y=y_)
            self.userPtsX.append(int(x_))
            self.userPtsY.append(int(y_))
            self.userPtsZ.append(int(z_))
            xList = np.hstack(self.userPtsX)
            yList = np.hstack(self.userPtsY)
            zList = np.hstack(self.userPtsZ)
            self.pointsPlot.mlab_source.reset(x=self.userPtsX,y=self.userPtsY,z=self.userPtsZ)
            # display connections:
            if len(self.userPtsX) != 1:
                ###self.edgesPlot1.mlab_source.reset(x=xList,y=yList,z=zList)
                # path integral:
                i = len(xList)
                self.connections.append(np.vstack(
                           [np.arange(i-2, i-1),
                            np.arange(i-1, i)]
                                ).T)
                self.pointsPlot.mlab_source.dataset.lines = self.connections
                xPair = (int(xList[i-2]), int(xList[i-1]))
                yPair = (int(yList[i-2]), int(yList[i-1]))
                if (xList[i-2] == xList[i-1]) and (yList[i-2] == yList[i-1]):
                    pass
                else:
                    linePoints = self.ptsLine(
                                        (xPair[0]+(np.shape(self.LS2d)[0]/2)),
                                        (yPair[0]+(np.shape(self.LS2d)[1]/2)),
                                        (xPair[1]+(np.shape(self.LS2d)[0]/2)),
                                        (yPair[1]+(np.shape(self.LS2d)[1]/2)))
                    if len(self.userPtsX) <= 2: # include origin point
                        self.allBlocks1.append(linePoints)
                    elif len(self.userPtsX) > 2: # prevents overcounting last point in previous line
                        linePoints.pop(0) 
                        self.allBlocks1.append(linePoints)
                    self.energy_width(np.vstack(self.allBlocks1)) #calculate energy integral for varying widths                    
    ...
    def _path_width_fired(self):
        self.energy_width(np.vstack(self.allBlocks1))
    
    @on_trait_change('self.path_width')
    def energy_width(self, allBlocks1):
        pixelWidth = int(self.path_width)
        if self.path_width == 1:
            pixelWidth = float(0.6) #1 pixel across (tube)
        if self.path_width == 3:
            pixelWidth = float(1.6) #3 pixels across (tube)
        if self.path_width == 5:
            pixelWidth = float(2.6) #5 pixels across (tube)
        self.edgesPlot2.filter.radius = pixelWidth
        v = self.scene4.mlab.view(figure=Mayavi_LS.fig4)
        pixelWidth = self.path_width #reset for pixel calculations
        self.scene4.mlab.view(distance=0, figure=Mayavi_LS.fig4)
        self.scene4.mlab.view(*v, figure=Mayavi_LS.fig4) #avoids visual glitch
        p_width3 = [] #pixel tracker, blocks = 3x3
        p_width5 = [] #pixel tracker, blocks = 5x5
        e_width1 = [] #energy tracker, blocks = 1
        e_width3 = [] #energy tracker, blocks = 3x3
        e_width5 = [] #energy tracker, blocks = 5x5
        for i in range(len(allBlocks1)):
            j, k = allBlocks1[i]
            # center pixel:
            C = self.LS2d[j,k]
            p_width3.append([j,k])
            p_width5.append([j,k])
            # first pixel border (used in 3x3 and 5x5):
            try:
                NW0 = self.LS2d[j-1,k-1]
                p_width3.append([j-1,k-1])
                p_width5.append([j-1,k-1])
            except IndexError:
                NW0 = 0
            try:
                N0 = self.LS2d[j-1,k]
                p_width3.append([j-1,k])
                p_width5.append([j-1,k])
            except IndexError:
                N0 = 0
            try:
                NE0 = self.LS2d[j-1,k+1]
                p_width3.append([j-1,k+1])
                p_width5.append([j-1,k+1])
            except IndexError:
                NE0 = 0
            try:
                W0 = self.LS2d[j,k-1]
                p_width3.append([j,k-1])
                p_width5.append([j,k-1])
            except IndexError:
                W0 = 0
            try:
                E0 = self.LS2d[j,k+1]
            ...
            try:
                ...
                S0 = 0
            try:
                SE0 = self.LS2d[j+1,k+1]
                p_width3.append([j+1,k+1])
                p_width5.append([j+1,k+1])
            except IndexError:
                SE0 = 0
            # second pixel border (used in 5x5):
            try:
                NWW1 = self.LS2d[j-2,k-2]
                p_width5.append([j-2,k-2])
            except IndexError:
                NWW1 = 0
            try:
                NW1 = self.LS2d[j-2,k-1]
                p_width5.append([j-2,k-1])
            except IndexError:
                NW1 = 0
            try:
                N1 = self.LS2d[j-2,k]
                p_width5.append([j-2,k])
            except IndexError:
                N1 = 0
            try:
                NE1 = self.LS2d[j-2,k+1]
                p_width5.append([j-2,k+1])
            except IndexError:
                NE1 = 0
            try:
                NEE1 = self.LS2d[j-2,k+2]
                p_width5.append([j-2,k+2])
            except IndexError:
                NEE1 = 0
            try:
                WN1 = self.LS2d[j-1,k-2]
                p_width5.append([j-1,k-2])
            except IndexError:
                WN1 = 0
            try:
                EN1 = self.LS2d[j-1,k+2]
                p_width5.append([j-1,k+2])
            except IndexError:
                EN1 = 0
            try:
                W1 = self.LS2d[j,k-2]
                p_width5.append([j,k-2])
            except IndexError:
                W1 = 0
            try:
                E1 = self.LS2d[j,k+2]
                ...
                E1 = 0
            try:
                WS1 = self.LS2d[j+1,k-2]
                p_width5.append([j+1,k-2])
            except IndexError:
                WS1 = 0
            try:
                ES1 = self.LS2d[j+1,k+2]
                p_width5.append([j+1,k+2])
            except IndexError:
                ES1 = 0
            try:
                SWW1 = self.LS2d[j+2,k-2]
                p_width5.append([j+2,k-2])
            except IndexError:
                SWW1 = 0
            try:
                SW1 = self.LS2d[j+2,k-1]
                p_width5.append([j+2,k-1])
            except IndexError:
                SW1 = 0
            try:
                ...
                S1 = 0
            try:
                SE1 = self.LS2d[j+2,k+1]
                p_width5.append([j+2,k+1])
            except IndexError:
                SE1 = 0
            try:
                SEE1 = self.LS2d[j+2,k+2]
                p_width5.append([j+2,k+2])
            except IndexError:
                SEE1 = 0
            # averaging based on width:
            if pixelWidth == 1:
                e_width1.append(C)
            if pixelWidth == 3:
                e_width3.append((C+NW0+N0+NE0+W0+E0+SW0+S0+SE0)/9)
            if pixelWidth == 5:
                e_width5.append((C+NW0+N0+NE0+W0+E0+SW0+S0+SE0+\
                                 NWW1+NW1+N1+NE1+NEE1+WN1+EN1+W1+\
                                 E1+WS1+ES1+SWW1+SW1+S1+SE1+SEE1)/25)

        self.allBlocks3.append(p_width3)
        self.allBlocks5.append(p_width5)
        
        if pixelWidth == 1:
            self.custom_path_integral = round(sum(e_width1),2)
            return
        if pixelWidth == 3:
            self.custom_path_integral = round(sum(e_width3),2)
            return
        if pixelWidth == 5:
            self.custom_path_integral = round(sum(e_width5),2)
            return
        

    view = View(HGroup(
                    Group(
                        Item('scene4', style='custom',
                            editor=SceneEditor(scene_class=MayaviScene), #=MayaviScene
                            width=450, height=100, show_label=False
                            ),
                            VGroup(
                                HGroup(
                                    HGroup(
                                        Item('reset_points',springy=True,show_label=False,
                                             tooltip='Reset all user-defined points in the plot above.',
                                             enabled_when='click_counter > 0',
                                             ),
                                        Item('energy_path',springy=True,show_label=False,
                                             tooltip='Plot the energy for the coupled coordinates of the chosen path.',
                                             enabled_when='click_counter > 1',
                                             ),
                                        Item('path_width',springy=False,show_label=True,
                                             tooltip='Change the pixel width of the path above.',
                                             enabled_when='click_counter > 1',
                                             ),
                                        Item('pixelsSqr',springy=False,show_label=False,style='readonly',
                                             ),
                                        ),
                                    show_border=True,
                    ...
                    VGroup(
                        # custom path panel:
                        HGroup( 
                            Item('custom_title',springy=True,show_label=False,
                                 style='readonly',
                                 style_sheet='*{font-size:13px; qproperty-alignment:AlignCenter}'
                                 ),
                            ),
                        VGroup(
                            HGroup(
                                Item(' '),
                                Item('custom_path_integral',springy=True,show_label=True,
                                     editor=TextEditor(evaluate=int),height=10,width=25,
                                     tooltip='The combined energy of all user-defined path segments on the energy landscape.',
                                     enabled_when='click_counter < 0',
                                     ),
                                Item('units',springy=True,show_label=False,style='readonly',
                                     ),
                                ),
                            HGroup(
                                Item('custom_save',springy=True,show_label=False,
                                     tooltip='Save user-defined path information to file.',
                                     enabled_when='click_counter > 1'),
                                Item('user_final',springy=True,show_label=False,
                                     tooltip='Generate the structural 3d sequence for the custom path.',
                                     enabled_when='click_counter > 1',
                                     ),
                                ),
                            show_border=True,
                        ...
                        # least action panel:
                        HGroup( 
                            Item('action_title',springy=True,show_label=False,
                                 style='readonly',
                                 style_sheet='*{font-size:13px; qproperty-alignment:AlignCenter}'
                                 ),
                            ),
                        VGroup(
                            HGroup(
                                Item('action_compute',springy=True,show_label=False,
                                     tooltip='Compute the path of least action between the selected points.',
                                     enabled_when='click_counter > 1',
                                     ),
                                ),
                            HGroup(
                                Item(' '),
                                Item('least_action_path_integral',springy=True,show_label=True,
                                     editor=TextEditor(evaluate=int),height=10,width=50,
                                     tooltip='The combined energy of the computed path of least action.',
                                     enabled_when='click_counter < 0',
                                     ),
                                Item('units',springy=True,show_label=False,style='readonly',
                                     ),
                            ...
                            HGroup(
                                Item('action_save',springy=True,show_label=False,
                                     tooltip='Save least action path information to file.',
                                     enabled_when='least_action_output > 0'),
                                Item('LA_final',springy=True,show_label=False,
                                     tooltip='Generate the structural 3d sequence for the least action path.',
                                     enabled_when='least_action_output > 0',
                                     ),
                                ),
                            show_border=True,
...
class P1(QtGui.QWidget):
    ...
    user_width = 0.0
    S2rescale = 0.0
    relion_data = True #True for .star
    q = []
    S2 = []
    df = []
    CG = []
    ...
    def __init__(self, parent=None):
        ...
        def choose_avgVol():
            ...
            if fileName:
                ...
                with mrcfile.open(P1.user_volume, mode='r+') as mrc:
                    ...
                    nx = mrc.header.nx #number of x-slices
                    ny = mrc.header.ny #number of y-slices
                    nz = mrc.header.nz #number of z-slices
                # measure maximum contour radius from center:
                nz_slices = np.zeros((nx,ny,nz))
                nz_traces = np.zeros((nx,ny,nz))
                xmax = []
                ymax = []
                xymax = []
                vmax = []

                for i in range(0,20): #find relative scalar distribution of datafile
                    vmax.append(np.amax(P1.df_vol[int(int(nx/2)-10+i)][:][:]))
                vmax = np.amax(vmax)

                for i in range(0,nz):
                    nz_slices[i] = P1.df_vol[i][:][:] #df[z][y][x], slice up data along z
                    nz_traces[i] = np.clip(nz_slices[i], .01, vmax, out=nz_slices[i]) #create contour thresholds

                    for j in range(0,nx):
                        if nz_traces[i][j,int(ny/2)] > .6*vmax: #may need tweaking
                            dist_x = int(np.absolute(nx/2 - j))
                            xmax.append(int(dist_x))

                    for k in range(0,ny):
                        if nz_traces[i][int(nx/2),k] > .6*vmax: #may need tweaking
                            dist_y = int(np.absolute(ny/2 - k))
                            ymax.append(int(dist_y))

                if len(xmax) and len(ymax):
                    xmax = np.amax(xmax)
                    ymax = np.amax(ymax)
                    xymax = [xmax,ymax]
                    xymax = int(np.amax(xymax)) #longest contour radius from center of cube

                elif len(xmax) and not len(ymax):
                    xymax = int(np.amax(xmax))

                elif not len(xmax) and len(ymax):
                    xymax = int(np.amax(ymax))
                    
                else:
                    xymax = float(nx)/2.

                P1.S2rescale = (2.*xymax/float(nx)) #used in plot to rescale S2 to match scale of contour volume
        ...
        self.connect(self.button_browse1, QtCore.SIGNAL('clicked()'), choose_avgVol)
 ...
 # alignment input:
        ...
        self.connect(self.button_browse3, QtCore.SIGNAL('clicked()'), choose_align)
        ...
        self.connect(self.button_browse2, QtCore.SIGNAL('clicked()'), choose_imgStack)
        ...
        self.connect(self.button_browseM, QtCore.SIGNAL('clicked()'), choose_maskVol)
        ...
        def choose_dir():
            ...
            if fileName:
                P1.entry_dir.setDisabled(False)
                P1.entry_dir.setText(fileName)
                P1.entry_dir.setDisabled(True)
                P1.user_directory = fileName
            else:
                P1.entry_dir.setDisabled(False)
                P1.entry_dir.setText(pyDir)
                P1.entry_dir.setDisabled(True)
                P1.user_directory = pyDir
            p.user_dir = P1.user_directory
            '''
        ...
        def choose_name():
            ...
            if ok:
                if (0 < len(text) <= 20) and (re.match('^[A-Za-z0-9-_]*$', text) != None):
                    P1.user_name = text
        ...
        self.connect(self.button_browse4, QtCore.SIGNAL('clicked()'), choose_name)
...
class P3(QtGui.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        def choose_dimensions():
            ...
            if P4.recompute == 1: #overwrite warning
                msg = 'The energy landscape has been previously computed under a different\
                        ...
                        all data rendered after the <i>Eigenfunctions</i> tab must be overwritten. \
                        <br /><br />\
                ...
                if reply == QtGui.QMessageBox.Yes:
                    ...
                    rcFile1 = os.path.join(P1.user_directory, 'outputs_{}/CC/CC_file'.format(p.proj_name))
                    rcDir1 = os.path.join(P1.user_directory, 'outputs_{}/CC/CC_meas'.format(p.proj_name))
                    ...
                    elDir1 = os.path.join(P1.user_directory, 'outputs_{}/ELConc50'.format(p.proj_name))
                    elDir2 = os.path.join(P1.user_directory, 'outputs_{}/ELConc50/OM'.format(p.proj_name))
                    outDir1 = os.path.join(P1.user_directory, 'outputs_{}/out'.format(p.proj_name))

                    if os.path.isfile(rcFile1):
                        os.remove(rcFile1)
                    if os.path.isdir(rcDir1):
                        shutil.rmtree(rcDir1)
                    if os.path.isdir(rcDir2):
                        shutil.rmtree(rcDir2)
                    if os.path.isdir(rcDir3):
                        shutil.rmtree(rcDir3)
                    if os.path.isdir(elDir1):
                        shutil.rmtree(elDir1)
                        os.makedirs(elDir1)
                        os.makedirs(elDir2)
                    ...
                    if P3.dictGen == True: #if P4 widget-dictionary has been created
                        if P3.user_dimensions == 1:
                            ...
                            for i in range(1,P3.PrD_total+1):
                                ...
                                P4.senses2All[i].setToolTip('CC2: Confirm arrow of time for selected topos.')
        ...
        self.label_psi = QtGui.QLabel('Eigenfunctions')
        ...
        P3.entry_psi.setToolTip('The number of eigenfunctions to compute.')
        ...
        P3.button_toP4 = QtGui.QPushButton('View Eigenfunctions', self)
    ...
    def calcP4(self):
        ...
        # PREPARE P4:
        ####################################
        # PrD threshold on Alignment data:
        fname = os.path.join(P1.user_directory, 'outputs_{}/topos/Euler_PrD/PrD_map.txt'.format(p.proj_name))
        data = []
        with open(fname) as values:
            for column in zip(*[line for line in csv.reader(values, dialect="excel-tab")]):
        ...
        #Erg1dMain.entry_PrD.setMaximum(P3.PrD_total) #updates P6
        #Erg1dMain.entry_PrD.setSuffix('  /  %s' % (P3.PrD_total))
        P4.viz2.update_scene3()
        ...
        for i in range(1,P3.PrD_total+1):
            ...
            sense1.setToolTip('CC1: Confirm arrow of time for selected topos.')
    ...
    def start_task2(self):
        ...
        task2 = threading.Thread(target=covar3d.op,
    ...
    def start_task4(self):
        ...
        task4 = threading.Thread(target=empty.op,
...
class P4(QtGui.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        P4.connect(self.button_bandwidth, QtCore.SIGNAL('clicked()'), self.bandwidth)
        ...
        P4.connect(self.button_eigSpec, QtCore.SIGNAL('clicked()'), eigSpectrum)
        P4.layoutR.addWidget(P4.button_eigSpec, 6, 9, 1, 1)
        P4.button_eigSpec.show()

        P4.button_viewAvg = QtGui.QPushButton('2D Class Average')
        P4.connect(self.button_viewAvg, QtCore.SIGNAL('clicked()'), classAvg)
        ...
        P4.connect(self.button_compareMov, QtCore.SIGNAL('clicked()'), self.CC_vid2)
        ...
        def anchorCheck():
            ...
            #SECTION NEEDED FOR RE-DOING GRAPH IF TRASH LIST DIVIDED ANY ISLANDS INTO SUB-ISLANDS
            ...
            if anch_sum < anchorsMin:    
                ...
                if sorted(np.unique(P3.col)) == sorted(np.unique(colors)):
                    ...
                    reply = box.exec_()
                    if reply == QtGui.QMessageBox.Yes:
                        p.anch_list = P4.anch_list #PrD,CC1,S1 for 1D; PrD,CC1,S1,CC2,S2 for 2D
                        anchInputs = os.path.join(P1.user_directory, 'outputs_{}/CC/user_anchors.txt'.format(p.proj_name))
                        np.savetxt(anchInputs, P4.anch_list, fmt='%i', delimiter='\t')
                        gotoP5(self)
                        if P4.recompute == 0:
        ...
        self.connect(self.btn_PDsele, QtCore.SIGNAL('clicked()'), self.PDSeleViz)
        ...
        self.connect(P4.btn_finOut, QtCore.SIGNAL('clicked()'), anchorCheck)
...
class anchTable(QtGui.QTableWidget):
    ...
    def BuildTable(self, values):
        ...
        if P3.user_dimensions == 1:
            headers = ['PD','CC','Sense','CC']
        elif P3.user_dimensions == 2:
            headers = ['PD','CC1','Sense1','CC2','Sense2','CC']
...
# inputs section:
        ...
        self.label_edgeMask1 = QtGui.QLabel('')
        self.label_edgeMask1.setMargin(20)
        self.label_edgeMask1.setLineWidth(1)
        self.label_edgeMask1.setFrameStyle(QtGui.QFrame.Panel | QtGui.QFrame.Sunken)
        layout.addWidget(self.label_edgeMask1, 2, 1, 1, 2)
        self.label_edgeMask1.show()

        self.label_mask = QtGui.QLabel('Optical Flow Masking')
        self.label_mask.setFont(font_standard)
        self.label_mask.setMargin(20)
        self.label_mask.setFrameStyle(QtGui.QFrame.Box | QtGui.QFrame.Sunken)
        self.label_mask.setAlignment(QtCore.Qt.AlignCenter | QtCore.Qt.AlignVCenter)
        layout.addWidget(self.label_mask, 2, 1, 1, 1)
        self.label_mask.show()

        P5.btn_mask = QtGui.QPushButton(' Choose Mask ', self)
        P5.btn_mask.clicked.connect(self.ViewMaskViz)
        layout.addWidget(P5.btn_mask, 2, 2, 1, 1, QtCore.Qt.AlignLeft | QtCore.Qt.AlignVCenter)
        P5.btn_mask.show()
        ...
        self.label_Hline2.setMargin(0)
        self.label_Hline2.setFrameStyle(QtGui.QFrame.HLine | QtGui.QFrame.Sunken)
        layout.addWidget(self.label_Hline2, 7, 1, 1, 6, QtCore.Qt.AlignVCenter)
        self.label_Hline2.show()

        P5.button_toP6 = QtGui.QPushButton('View Energy Landscape', self)
        P5.button_toP6.clicked.connect(gotoP6)
        layout.addWidget(P5.button_toP6, 8, 3, 1, 2)
        P5.button_toP6.setDisabled(True)
        P5.button_toP6.show()
        ...
        self.label_space.setFont(font_standard)
        self.label_space.setMargin(0)
        layout.addWidget(self.label_space, 9, 0, 7, 4, QtCore.Qt.AlignVCenter)
        self.label_space.show()

    def ViewMaskViz(self):
        global ViewMask_window
        try:
            ViewMask_window.close()
        except:
            pass
        ViewMask_window = ViewMaskMain()
        ViewMask_window.setMinimumSize(500, 500)
        ViewMask_window.setWindowTitle('Optical Flow Mask')
        ViewMask_window.show()
    ...
    def start_task6(self):
        ...
        task6 = threading.Thread(target=empty.op, args=(self.progress6Changed, ))
        '''
        if P3.user_dimensions == 1:
    ...
    @QtCore.Slot(int)
    def on_progress6Changed(self, val):
...
################################################################################
# ui page 5: ENERGY PATH/LANDSCAPE
    ...
    def start_task7(self):
        if Erg1dMain.reprepare == 1:
            ...
            if reply == QtGui.QMessageBox.Yes:
                ...
                task7 = threading.Thread(target=empty.op, args=(self.progress7Changed, ))
                task7.daemon = True
                task7.start()
...
class Erg2dCanvas(QtGui.QWidget):
    custom_save = ''
    custom_gen = ''
    action_save = ''
    action_gen = ''
    ...
    def __init__(self, parent=None):
        super(Erg2dCanvas, self).__init__(parent)
        layout = QtGui.QGridLayout(self)
        layout.setContentsMargins(20,20,20,20)
        layout.setSpacing(10)

        self.viz3 = Mayavi_LS()
        self.ui3 = self.viz3.edit_traits(parent=self, kind='subpanel').control
        layout.addWidget(self.ui3, 0, 0, 1, 1)
        self.ui3.setParent(self)

        # custom path:
        global customSaveDialog 
        def customSaveDialog():
            msg = 'Performing this action will save all information obtained from \
                    the current path into the user directory, including point coordinates \
                    and respective energies.\
                    <br /><br />\
                    Do you want to proceed?' 
            box = QtGui.QMessageBox(self)
            box.setWindowTitle('%s' % progname)
            ...
            box.setStandardButtons(QtGui.QMessageBox.Yes|QtGui.QMessageBox.No)
            box.setInformativeText(msg)
            P6.custom_save = box.exec_()

        # least action path:
        global leastActionDialog
        def leastActionDialog():
            msg = 'Performing this action will calculate the minimum energy \
                    path between the selected points on the energy landscape.\
                    <br /><br />\
                    Computation times will vary. Do you want to proceed?'
            box = QtGui.QMessageBox(self)
            box.setWindowTitle('%s' % progname)
            box.setText('<b>Compute Path of Least Action</b>')
            box.setFont(font_standard)
            box.setIcon(QtGui.QMessageBox.Question)
            box.setInformativeText(msg)
            box.setStandardButtons(QtGui.QMessageBox.Yes|QtGui.QMessageBox.No)
            P6.action_start = box.exec_()

        global leastActionError 
        def leastActionError():
            msg = 'Exactly two points must be selected to perform this calculation. \
                    <br /><br />\
                    To proceed, first pick only a start point and end point on the \
                    energy landscape.' 
            box = QtGui.QMessageBox(self)
            box.setWindowTitle('%s Error' % progname)
            box.setText('<b>Input Error</b>')
        ...
        global actionSaveDialog 
        def actionSaveDialog():
            msg = 'Performing this action will save all information obtained from \
                    the minumum energy path into the user directory, including point \
                    coordinates and respective energies.\
                    <br /><br />\
                    Do you want to proceed?' 
            box = QtGui.QMessageBox(self)
            box.setWindowTitle('%s' % progname)
            box.setText('<b>Save Least Action Path</b>')
            box.setFont(font_standard)
            box.setIcon(QtGui.QMessageBox.Question)
            box.setStandardButtons(QtGui.QMessageBox.Yes|QtGui.QMessageBox.No)
            box.setInformativeText(msg)
            P6.action_save = box.exec_()

        global userFinalDialog 
        def userFinalDialog():
            ...
            box.setStandardButtons(QtGui.QMessageBox.Yes|QtGui.QMessageBox.No)
            box.setInformativeText(msg)
            P6.custom_gen = box.exec_()

        global laFinalDialog 
        def laFinalDialog():
            msg = 'Performing this action will generate the 3D structural sequence \
                    for the computed path of least action.\
                    <br /><br />\
                    Do you want to proceed?' 
            box = QtGui.QMessageBox(self)
            box.setWindowTitle('%s' % progname)
            ...
            box.setStandardButtons(QtGui.QMessageBox.Yes|QtGui.QMessageBox.No)
            box.setInformativeText(msg)
            P6.action_gen = box.exec_()

        global completeDialog 
        def completeDialog():
            msg = '3D structural sequence exported successfully.'
            box = QtGui.QMessageBox(self)
            box.setWindowTitle('%s' % progname)
    ...
    def anchorList(self):
        ...
        if idx_sum == 0:
            ...
            box.setInformativeText('No PD anchors have been selected.\
                                    ...
                                    on the left side of the <i>Eigenfunctions</i> tab.')
            box.setStandardButtons(QtGui.QMessageBox.Ok)
            box.setDefaultButton(QtGui.QMessageBox.Ok)
            ret = box.exec_()
    ...
    def anchorSave(self):
        ...
        if anch_sum == 0:
            ...
            if reply == QtGui.QMessageBox.Yes:
                ...
                np.savetxt(tempAnchInputs, temp_anch_list, fmt='%i', delimiter='\t')
        ...
        if anch_sum == 0:
            ...
            if PDSeleCanvas1.fname:
                try:
                    ...
                    box.setFont(font_standard)
                    msg = 'Previous anchor selections have been loaded on the <i>Eigenfunctions</i> tab.'
                    box.setStandardButtons(QtGui.QMessageBox.Ok)
                    box.setInformativeText(msg)
                    reply = box.exec_()
        ...
        if trash_sum == 0:
            ...
            if fname:
                try:
                    ...
                    box.setText('<b>Loading Complete</b>')
                    box.setFont(font_standard)
                    msg = 'Previous removal selections have been loaded on the <i>Eigenfunctions</i> tab.'
                    box.setStandardButtons(QtGui.QMessageBox.Ok)
                    box.setInformativeText(msg)
                    reply = box.exec_()
...
class Vid2Canvas(QtGui.QDialog):
    ...
    def __init__(self, parent=None):
        ...
        picImg = Image.open(picDir)
        picSize = picImg.size
        Vid2Canvas.blank = np.zeros([picSize[0],picSize[1],3], dtype=np.uint8)
        #################################################
            ...
            for file in sorted(files):
                if not file.startswith('.'): #ignore hidden files
                    if file.endswith(".png"):
                        ...
                        #self.imgs1.append(misc.imread(self.img_paths1[i]))
                        #self.imgs2.append(misc.imread(self.img_paths1[i]))
        ...
        self.sense1.setToolTip('Arrow of time for selected movie.')
        ...
        self.sense2.setToolTip('Arrow of time for selected movie.')
        ...
        Vid2Canvas.buttonForward = QtGui.QPushButton(ur'\u25BB') #u25B6, u25BA
        ...
        Vid2Canvas.buttonPause = QtGui.QPushButton(ur'\u25A1') #u25A0, u25FC
        ...
        Vid2Canvas.buttonBackward = QtGui.QPushButton(ur'\u25C5') #u25C0, u25C4
        ...
        layout.addWidget(self.slider,6,2,1,7) #6,0,1,11
                ...
                for file in sorted(files):
                    if not file.startswith('.'): #ignore hidden files
                        if file.endswith(".png"):
                            ...
                            self.imgs2.append(misc.imread(self.img_paths2[i]))
                            i += 1
...
class Manifold2dCanvas(QtGui.QDialog):
    eig_current = 1 #current eigenvector window
    eig_compare1 = 2 #maximum eigenvalue s.t. != eig_current
    eig_compare2 = 3 #next highest eigenvalue
    coords = [] #user coordinate picks
    ...
    def __init__(self, parent=None):
        ...
        self.ax.autoscale()
        #self.ax.grid(linestyle='-', linewidth='0.5', color='lightgray', alpha=0.2)
    ...
    def reset(self):
        if len(self.ax.lines) != 0:
            if self.connected == 0:
                ...
                for i in range(1,(len(self.coords)*2)+1):
                    del(self.ax.lines[-1]) #delete all vertices and edges
                self.connected = 0
        else:
            self.connected = 0
    ...
    def connect(self):
        if len(self.coords) > 2:
            ...
            ax.plot([self.coords[0][0],self.coords[-1][0]],
    ...
    def remove(self):
        Manifold2dCanvas.pts_new = [] #reset cropped points if re-clicked
        ...
        path = pltPath.Path(self.coords, codes)
        inside = path.contains_points(Manifold2dCanvas.pts_orig,radius=1e-9)
        ...
        for i in inside:
            index += 1
            if i == False:
                Manifold2dCanvas.pts_new.append(Manifold2dCanvas.pts_orig[index-1])
            else:
                sums += 1 #number of encircled points

        delta = len(Manifold2dCanvas.pts_new)

        if delta >= p.PDsizeThL:
            # crop out points, redraw and resize figure:
            self.ax.clear()
            for i in Manifold2dCanvas.pts_new:
                x,y = i
                self.ax.scatter(x,y,s=1,c='#1f77b4') #plot initial data, C0
            for tick in self.ax.xaxis.get_major_ticks():
                tick.label.set_fontsize(4)
    ...
    def rebed(self):
        ...
        if reply == QtGui.QMessageBox.Yes:
            ...
            #embedd.op(Manifold2dCanvas.pts_orig, Manifold2dCanvas.pts_new, P4.user_PrD - 1) #updates all manifold files for PD
            self.start_task1()
    ...
    def view(self): #view average of all images in encircled region
        ...
        for i in inside:
            index += 1
            if i == True:
                Manifold2dCanvas.pts_encircled.append(Manifold2dCanvas.pts_orig[index-1])
        ...
        if self.connected == 0:
            ...
            if ix != None and iy != None:
                ...
                if len(self.coords) > 1:
                    x0, y0 = self.coords[-2][0], self.coords[-2][1]
...
class BandwidthCanvas(FigureCanvas): 
    ...
    def plot(self):
        ...
        ax = self.figure.add_subplot(111)
        ax.clear()
        ax.scatter(BandwidthCanvas.logEps, BandwidthCanvas.logSumWij, s=1, c='#1f77b4', edgecolor='#1f77b4', zorder=.1) #blue
        ax.plot(BandwidthCanvas.logEps, fun(BandwidthCanvas.logEps,
                                                 ...
                                                 BandwidthCanvas.popt[3]), c='red', linewidth=.5, zorder=.2)    
        ax.axvline(-(BandwidthCanvas.popt[1] / BandwidthCanvas.popt[0]), color='lightgray', alpha=.5, linewidth=1, linestyle='-', zorder=0)

        
        effDim = 2 * np.fabs(BandwidthCanvas.popt[0]) #see Fergusson paper (supplementary material)
        textstr = '\n'.join((
                    r'$y=%.2f + %.2f tanh(%.2fx + %.2f)$' % (BandwidthCanvas.popt[3], BandwidthCanvas.popt[2], BandwidthCanvas.popt[0], BandwidthCanvas.popt[1]),
                    r'$\mathrm{Effective \ Dimensionality=%.2f}$' % (effDim, ),
...
class ChronosCanvas(QtGui.QDialog):
    def __init__(self, parent=None):
        ...
        self.ax1.set_xticks(np.arange(0, len(chronos1)+1, 100))
        ...
        self.ax2.set_xticks(np.arange(0, len(chronos1)+1, 100))
        ...
        self.ax3.set_xticks(np.arange(0, len(chronos1)+1, 100))
        ...
        self.ax4.set_xticks(np.arange(0, len(chronos1)+1, 100))
        ...
        self.ax5.set_xticks(np.arange(0, len(chronos1)+1, 100))
        ...
        self.ax6.set_xticks(np.arange(0, len(chronos1)+1, 100))
        ...
        self.ax7.set_xticks(np.arange(0, len(chronos1)+1, 100))
        ...
        self.ax8.set_xticks(np.arange(0, len(chronos1)+1, 100))
...
class Thresh_Viz(QtGui.QMainWindow):
    ...
    def onTabChange(self, i):
        if i == 1: #signals when view switched to tab 2
            ...
            thresh = ThreshFinalCanvas.axes.scatter(temp_phi, temp_theta, edgecolor='k', linewidth=.5, c=temp_occ, s=5, cmap=cm.jet)
            ...
            ThreshFinalCanvas.cbar.set_clim(0.,float(np.amax(P1.all_occ)))
...
################################################################################
# plot optical flow mask options:

class ViewMaskMain(QtGui.QMainWindow):
    pd = 1
    openWindow = False

    def __init__(self):
        super(ViewMaskMain, self).__init__()
        self.left = 10
        self.top = 10
        ViewMaskMain.figure = Figure(dpi=200, facecolor='w', edgecolor='w')
        ViewMaskMain.ax = ViewMaskMain.figure.add_axes([0,0,1,1])

        ViewMaskMain.canvas = FigureCanvas(ViewMaskMain.figure)
        ViewMaskMain.toolbar = NavigationToolbar(ViewMaskMain.canvas, self)
        self.initUI()
    ...
    def initUI(self):
        ...
        ViewMaskMain.entry_PrD = QtGui.QSpinBox(self)
        ViewMaskMain.entry_PrD.setMinimum(1)
        ViewMaskMain.entry_PrD.setMaximum(P3.PrD_total)
        ViewMaskMain.entry_PrD.setPrefix('PD: ')
        ViewMaskMain.entry_PrD.setSuffix('  /  %s' % (P3.PrD_total))
        ViewMaskMain.entry_PrD.valueChanged.connect(self.set_PrD)
        ViewMaskMain.entry_PrD.setToolTip('Change the projection direction of the current view above.')

        ViewMaskMain.label_mask = QtGui.QLabel('Mask Type:')
        ViewMaskMain.label_mask.setFont(font_standard)
        ViewMaskMain.label_mask.setMargin(20)
        #ViewMaskMain.label_mask.setFrameStyle(QtGui.QFrame.Box | QtGui.QFrame.Sunken)
        ViewMaskMain.label_mask.setAlignment(QtCore.Qt.AlignRight)

        ViewMaskMain.entry_mask = QtGui.QComboBox(self)
        ViewMaskMain.entry_mask.setFont(font_standard)
        ViewMaskMain.entry_mask.addItem('None')
        ViewMaskMain.entry_mask.addItem('Annular')
        ViewMaskMain.entry_mask.addItem('Volumetric')
        ViewMaskMain.entry_mask.setToolTip('Choose mask type for optical flow.')
        ViewMaskMain.entry_mask.setCurrentIndex(p.opt_mask_type) #load in previous settings if re-opened
        ViewMaskMain.entry_mask.currentIndexChanged.connect(self.set_mask_type)

        ViewMaskMain.entry_param = QtGui.QDoubleSpinBox(self)
        ViewMaskMain.entry_param.setDecimals(0)
        ViewMaskMain.entry_param.setMinimum(0)

        if p.opt_mask_type == int(0):
            ViewMaskMain.entry_param.setValue(0)
            ViewMaskMain.entry_param.setDisabled(True)
            ViewMaskMain.entry_param.setPrefix('')
        if p.opt_mask_type == int(1):
            ViewMaskMain.entry_param.setMaximum(int(p.nPix/2))
            ViewMaskMain.entry_param.setDisabled(False)
            ViewMaskMain.entry_param.setValue(p.opt_mask_param)
            ViewMaskMain.entry_param.setPrefix('Radius: ')
        ViewMaskMain.entry_param.valueChanged.connect(self.set_mask_param)

        layout.addWidget(ViewMaskMain.toolbar, 0, 0, 1, 6, QtCore.Qt.AlignVCenter)
        layout.addWidget(ViewMaskMain.canvas, 1, 0, 1, 6, QtCore.Qt.AlignVCenter)
        layout.addWidget(ViewMaskMain.entry_PrD, 2, 1, 1, 1)
        layout.addWidget(ViewMaskMain.label_mask, 2, 2, 1, 1)
        layout.addWidget(ViewMaskMain.entry_mask, 2, 3, 1, 1)
        layout.addWidget(ViewMaskMain.entry_param, 2, 4, 1, 1)

        self.setWindowModality(QtCore.Qt.ApplicationModal) #freezes out parent window
        self.show()
        ViewMaskMain.openWindow = True

    def set_PrD(self):
        ViewMaskMain.pd = ViewMaskMain.entry_PrD.value()
        self.plot()

    def set_mask_type(self):
        if ViewMaskMain.entry_mask.currentText() == 'None':
            ViewMaskMain.entry_param.setPrefix('')
            ViewMaskMain.entry_param.setDisabled(True)
            ViewMaskMain.entry_param.setMinimum(0)
            ViewMaskMain.entry_param.setValue(0)
            p.opt_mask_param = int(0)
            p.opt_mask_type = int(0)
        else:
            ViewMaskMain.entry_param.setDisabled(False)
            ViewMaskMain.entry_param.setMinimum(1)
            if ViewMaskMain.entry_mask.currentText() == 'Annular':
                ViewMaskMain.entry_param.setPrefix('Radius: ')
                ViewMaskMain.entry_param.setMaximum(int(p.nPix/2))
                ViewMaskMain.entry_param.setValue(p.nPix/2)
                p.opt_mask_type = int(1)
                p.opt_mask_param = int(p.nPix/2)
            else:
                ViewMaskMain.entry_param.setPrefix('')
                ViewMaskMain.entry_param.setMaximum(0)
                ViewMaskMain.entry_param.setValue(0)
                p.opt_mask_type = int(2)
        self.plot()

    def set_mask_param(self):
        p.opt_mask_param = int(ViewMaskMain.entry_param.value())
        self.plot()
        
    def plot(self):
        ViewMaskMain.ax.clear()
        fname = os.path.join(P1.user_directory, 'outputs_%s/topos/PrD_%s/class_avg.png' % (p.proj_name, ViewMaskMain.pd))
        imgFull = mpimg.imread(fname)
        img = cv2.resize(imgFull, (p.nPix, p.nPix))
        
        if p.opt_mask_type == int(1): #1 for annular mask
            bg = []
            h, w = img.shape[:2]
            center = [int(w/2), int(h/2)]
            radius = p.opt_mask_param #int(h/2)
            Y, X = np.ogrid[:h,:w]
        ...
        if p.opt_mask_type == int(2): #2 for volumetric projection
            if p.mask_vol_file != '':
                img = projectMask.op(p.mask_vol_file,PD)
        
        ViewMaskMain.ax.set_title('2D Class Average', fontsize=6)
        ViewMaskMain.ax.imshow(img, cmap='gray')
        ViewMaskMain.ax.axis('off')
        ViewMaskMain.canvas.draw() #refresh canvas
        
    def closeEvent(self, ce): #message if user clicks to exit via window button
        if ViewMaskMain.openWindow == True:
            if p.opt_mask_type == int(0):
                mask_type = 'None'
                mask_param = 'N/A'
            elif p.opt_mask_type == int(1):
                mask_type = 'Annular'
                mask_param = p.opt_mask_param
            else:
                mask_type = 'Isosurface'
                mask_param = p.opt_mask_param
                
            msg = 'The following masking parameters have been defined for Optical Flow:\
                    <br /><br />\
                    <i>Mask Type</i>: %s\
                    <br />\
                    <i>Mask Range</i>: %s' % (mask_type, mask_param)
            box = QtGui.QMessageBox(self)
            box.setWindowTitle('Optical Flow Masking')
            box.setText('<b>Mask Settings</b>')
            box.setFont(font_standard)
            box.setIcon(QtGui.QMessageBox.Information)
            box.setInformativeText(msg)
            box.setStandardButtons(QtGui.QMessageBox.Ok)
            reply = box.exec_()
            ViewMaskMain.openWindow = False
...
class MainWindow(QtGui.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        helpMenu.addAction('&Eigenfunctions', self.guide_eigenfunctions)
        ...
        if MainWindow.inputsFile:
            ...
            if MainWindow.inputsFile.endswith('.pkl'):
                ...
                try:
                    ...
                    if p.resProj == 2: #processors & eigenfunctions entered; GetDistancesS2 complete
                        ...
                        print('\teigenfunctions: %s' % (p.num_psis))
    ...
    def onTabChange(self,i):
        ...
        if i == 3: #auto-update Mayavi_Rho figure
            ...
            if (MainWindow.S2_scale != MainWindow.S2_scale_prev) or (MainWindow.S2_iso != MainWindow.S2_iso_prev):
                P3.dictGen = False
                P4.viz2.update_S2()
                P4.viz2.update_scene3()
                P3.dictGen = True
    ...
    def guide_embedding(self):
        ...
        box.setInformativeText("<span style='font-weight:normal;'>\
                                ...
                                the user can navigate to the <i>Eigenfunctions</i> tab via the\
                                <i>View Eigenfunctions</i> button at the bottom of the screen.\
                                </span>")
        box.setStandardButtons(QtGui.QMessageBox.Ok)        
        ret = box.exec_()

    def guide_eigenfunctions(self):
        box = QtGui.QMessageBox(self)
        box.setWindowTitle('%s Help' % progname)
        box.setText('<b>Eigenfunctions Tab</b>')
        box.setFont(font_standard)
        box.setInformativeText("<span style='font-weight:normal;'>\
                                <b>Selecting PDs:</b>\
                                ...
                                from each PD - including that PD's set of unique eigenfunctions\
                                ...
                                <i>Belief Propagation</i> algorithm - in which the eigenfunctions are matched\
                                ...
                                <i>Set PD Anchors</i> box - making sure that only eigenfunctions of highest\
                                ...
                                motion can also be easily identified across its set of eigenfunctions? (etc.) </i>\
                                <br /><br />\
                                ...
                                eigenfunctions must instead be chosen for a set of highest certainty PDs.\
    ...
    def guide_dyn_1d(self):
        ...
        box.setInformativeText("<span style='font-weight:normal;'>\
            ...
            <br /><br />\
            Finally, the 3d reconstruction along this trajectory is automatically created\
            and saved within the <i>outputs</i> folder.\
            </span>")
        box.setStandardButtons(QtGui.QMessageBox.Ok)        
        ret = box.exec_()
    ...
    def guide_dyn_custom(self):
        box = QtGui.QMessageBox(self)
        box.setWindowTitle('%s Help' % progname)
        box.setText('<b>2D Dynamics Tab</b>')
        box.setFont(font_standard)
        box.setInformativeText('Custom data points can be selected by clicking directly on the energy landscape.\
            ...
            clicking <i>Generate Custom 3D Sequence</i>.')
        box.setStandardButtons(QtGui.QMessageBox.Ok)        
        ret = box.exec_()
    ...
    def guide_dyn_action(self):
        ...
        box.setText('<b>2D Dynamics Tab</b>')
        box.setFont(font_standard)
        box.setInformativeText('The path of least action can be computed between any two points on the energy landscape.\
            <br /><br />\
            Click the energy landscape once to choose a start point, then again at a new location to choose\
            an end point.<br /> <br /> \
            \
            Click the <i>Compute Path of Least Action</i> button to calculate the path of minimum energy \
            between the two chosen points.<br /> <br /> \
            \
            Finally, the 3d reconstruction along the trajectory can be created by clicking\
            <i>Generate LA 3D Sequence</i>.')
        box.setStandardButtons(QtGui.QMessageBox.Ok)        
        ret = box.exec_()
    ...
    def about(self):
        ...
        box.setText('<b>%s v.%s Python 2.7</b>' % (progname, progversion))
        box.setFont(font_standard)
...
if __name__ == '__main__':
    print('######################')
    print('#  %s' % (progname))
    print('#  v.%s Python 2.7' % (progversion))
    print('######################')
    print('')
