function array=trace(lambda,ind2,ind3,type,const, compare) % no typew
...
out=strcat('/data2/liaoh/new_with_covar_reduce_index_DHX_',type,'w/big_tot_matlab/tot_',num2str(1,'%05d'));
p = load(out);
% sizes
N1 = size(p.toti,2)
% N = sqrt(N1);
...
for n = 1:100
    ...
    for i = 1:I % for each view (block-ART)
        ...
        l = sel(j);
        % load tot
        out = strcat('/data2/liaoh/new_with_covar_reduce_index_DHX_',type,'w/big_tot_matlab/tot_',num2str(j,'%05d'));
        p = load(out);
        A = p.toti;
        if compare == 1
        ...
        %data = load(string);
        %data=data(:);
        string=strcat('covar/stats/scovar_',type,'w',num2str(l,'%05d'),'.txt'); % run
        data = load(string);
        if compare == 1
            string=strcat('covar/stats/scovar_',type,'w',num2str(l,'%05d'),'.txt'); % run
            data1 = load(string);
        ...
        %toc
        if compare == 1 %& n>1
            arr = weighted(array,ind3);
            %whos data1 A1 arr A array data
            resid1 = resid1 + norm(data1 - A1*arr);
        end
    ...
    var = get_diag3(arr,ind3);
    str = strcat('var_',num2str(const),'_3a',type,'w',num2str(n,'%02d'),'.spi');
    ...
    grs = grs/size(sel,1)
    report(n) = resid;
    string = strcat('report_',num2str(const),'_3a',type,'w',num2str(n,'%05d'));
    save(string,'report');
    %array_total = array;
    string = strcat('array_total_',num2str(const),'_3a',type,'w',num2str(n,'%05d'));
    save(string,'arr','-v7.3');
