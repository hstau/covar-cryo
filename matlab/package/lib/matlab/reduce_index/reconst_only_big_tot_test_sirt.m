...
for n = 1:100
    ...
    for i = 1:I % for each view (block-ART)
        ...
        % summing along unknowns
        Ai = sum(A,2); % Ai is column vector
        good = Ai > tiny;
        Ai = good.*Ai + (1-good).*tiny;
        % summing along data
        Aj = sum(A); % Aj is a row vector
        good = Aj > tiny;
        Aj = good.*Aj + (1-good).*tiny;
        Sj = Sj + Aj';
        % data = prj.cov_2d(:);
        % iteration
        dif = (data - A*array);
        ...
        dif = A'*dif;
        err = err + dif;
        res = norm(data - A*array);
        resid = resid + res*res;
    end
    array = array + lambda*err./Sj;
    %tic
    if (const == 1)
        array = array_constraints(array);
    end
    %toc
    
    % get the diag elemetns
    var = get_diag3(array,ind3);
    str = strcat('varSIRT_',num2str(const),'_3a',type,num2str(n,'%02d'),'.spi');
    ...
    save(string,'report');
    %array_total = array;
    string = strcat('array_totalSIRT_',num2str(const),'_3a',type,num2str(n,'%05d'));
