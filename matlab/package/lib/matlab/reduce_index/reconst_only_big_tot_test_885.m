...
% read angle sel file 
sel = readSPIDERdoc('covar/885/sel_ang.spi');
I = size(sel,1); 
% read one total
%out=strcat('/data2/liaoh/new_with_covar_reduce_index_DHX_fine/big_tot_matlab/tot_',num2str(1,'%05d'));
out=strcat('/data2/liaoh/new_with_covar_reduce_index_DHX_',type,'_885_030/big_tot_matlab/tot_',num2str(1,'%05d'));
p = load(out);
% sizes
N1 = size(p.toti,2)
% N = sqrt(N1);
...
for n = 1:30
    ...
    for i = 1:I % for each view (block-ART)
        j = J(i);
        l = sel(j);
        string=strcat('covar/885/selfiles/prj_sel_',num2str(l,'%05d'),'.spi');
        prj_sel = readSPIDERdoc(string);
        if (size(prj_sel,1) > th)
            % load tot
            out = strcat('/data2/liaoh/new_with_covar_reduce_index_DHX_',type,'_885_030/big_tot_matlab/tot_',num2str(j,'%05d'));
            p = load(out);
            A = p.toti;
            p = [];
            % size data
            ...
            %data = load(string);
            %data=data(:);
            string=strcat('covar/885/stats/ffnscovar_',type,num2str(l,'%05d'),'.txt'); % run
            data = load(string);
    ...
    str = strcat('ffn885var_',num2str(const),'_3a',type,num2str(n,'%02d'),'_',num2str(th,'%03d'),'.spi');
    ...
    grs = grs / cc
    report(n) = resid;
    string =   strcat('ffn885report_',num2str(const),'_3a',type,num2str(n,'%05d'),'_',num2str(th,'%03d'));
    save(string,'report');
    %array_total = array;
    string = strcat('ffn885array_total_',num2str(const),'_3a',type,num2str(n,'%05d'),'_',num2str(th,'%03d'));
