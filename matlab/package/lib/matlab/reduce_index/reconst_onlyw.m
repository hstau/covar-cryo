function [prj,array]=trace(ind2,ind3,lambda,amat,amatw,ft)
...
for n = 1:20
    ...
    for i = 1:I % for each view (block-ART)
        ...
        arr = weighted_amat(array,ind3,ft);
        resid = resid + norm(da - A1*arr);
    end
    resid
    report(n) = resid;
end
string = strcat('report_amatw_040_',num2str(ft,'%4.2f'));
save(string,'report','-ascii');
array = expand_volume(arr,ind3,v_dim);
string = strcat('matlab_result_amatw_040_',num2str(ft,'%4.2f'),'.spi');
writeSPIDERfile(string,array);
