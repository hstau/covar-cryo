function mat = trace(euler, v_dim, p_dim, ind2, ind3, i,ft)
...
for uu = 1:Mu % Mu2+1 
    for vv = 1:Mv % Mu2+1 
        ...
        rad2 = rx*rx + ry*ry;
        mat = gtrace(0, u1, v1, rad2, pp,unit, v_dim, p_dim, find2, ind3, factor, mat,ft);
        ...
        if mod(uu,2) == 1 && mod(vv,2) == 1
           ...
           v1 = floor(v1/2);
           rad2 = rx*rx + ry*ry;
        mat = gtrace(1, u1, v1, rad2, pp,unit, v_dim, p_dim, cind2, ind3, factor, mat,ft);           
        end
    end
end
...
% normalize the matrix NO!!
% mm = max(max(mat));
mat = mat/10;
    

function matn = gtrace(sp, u1, v1, rad2, pp, unit, v_dim, p_dim, ind2, ind3, factor, mat,ft)
Mu = p_dim(1);
Mv = p_dim(2);
Mu2 = Mu/2;
Mv2 = Mv/2;
...
ind = u1*Mv + v1 + 1; % use new indexingpot
ind = ind2(ind);      
if ind > 0            % if within mask
    ...
    l = ind_we(pos, v_dim, ind3, factor,ft);
    on = ones(size(l,1),1);
...
function  l = ind_we(pos,v_dim,ind3,factor,ft)
find3 = ind3{1};
cind3 = ind3{2};
l = [];
if size(pos,1) > 0
