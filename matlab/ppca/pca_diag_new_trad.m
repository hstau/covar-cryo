...
function [ E, A, S, Mu, V, cv, hp, lc ] = pca_diag( X, PM, V, voli, ncomp )
...
opts = struct( ...
    ...
    'cfstop',        [ 100 0 0],... % [] means no cost stop criteria
    'verbose',       1,...
    'bias',          0,...
    'autosave',      3600,...
...
for iter = 1:opts.maxiters
    ...
    if iter == 1
        lc.rms = rms; lc.cost = cost_old; % lc.prms = prms; 
        if opts.verbose
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [ EE, AA, SS, Mu, V, Av, Sv, Muv ] = InitParms( init, n1, n2, n3, voli, PM, ncomp )
AA = cell(2,1);
SS = cell(2,1);
EE = cell(2,1);
cur = 1; new = 2;
