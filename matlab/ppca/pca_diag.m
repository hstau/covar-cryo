...
function [ E, A, S, Mu, V, cv, hp, lc ] = pca_diag( X, PM, V, voli, ncomp, varargin )
...
defopts = struct( ...
    ...
    'cfstop',        [ 100 0 0 ],... % [] means no cost stop criteria %% chg
    'verbose',       1,...
    'bias',          0,... %% chg
    'autosave',      3600,...
...
[ EE, AA, SS, Mu, V, Av, Sv, Muv ] = InitParms( opts.init, n1, n2, n3, voli(:), PM, ncomp );
...
cur = 1; new = 2;
lrate = 1; lrateA = 1e-5; lrateS = 1e-5;
alpha = 2/3;
...
for iter = 1:opts.maxiters
    ...
    nor = norm(SS{new},'fro')/sqrt(size(SS{new},1)*size(SS{new},2)-1);
    SS{new} = SS{new}/nor;
    AA{new} = AA{new}*nor;
    nA = norm(AA{new},'fro')/sqrt(size(AA{new},1)*size(AA{new},2)-1)
    ...
    if cost < cost_old
        ...
        while cost > cost_old && sd_iter < 40
            ...
            lrateA = lrateA * 0.5;
            lrateS = lrateS * 0.5;
            EE{new} = (EE{new}+EE{cur})/2;
            AA{new} = PM*EE{new};
            %  AA{new} = (AA{new}+AA{cur})/2; chg
...
aux = cell(1,size(Sv,2)); %% Added
for j=1:size(Sv,2), aux{j}=diag(Sv(:,j)); end
Sv = aux;
%
aux = cell(1,size(Av,1)); %% Added
for j=1:size(Av,1), aux{j}=diag(Av(j,:)); end
Av = aux;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [ EE, AA, SS, Mu, V, Av, Sv, Muv ] = InitParms( init, n1, n2, n3, voli, PM, ncomp ) %% chg
...
if isstruct(init)
    if isfield( init, 'A' )
        ...
        EE{cur} = zeros(n3,ncomp);
        %EE{cur} = repmat(voli,[1,size(EE{cur},2)]);
        AA{cur} = full(PM*EE{cur});
    end
    if isfield( init, 'Av' )
        Av = init.Av;
    else
