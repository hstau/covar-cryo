...
string=strcat('covar/stats/rep_mask030',num2str(sel(1),'%05d'),ext);
...
for k = 1:I
    j = sel(k);
    string=strcat('covar/stats/rep_mask030',num2str(j,'%05d'),ext);
    ...
    for i = 1:size(mask(:))
        ...
        if mask(i) == 1 % !!!!!!!!!!!!!!!!!!!!!!!! CHECK HERE 
            find2(i,k) = 0;%-i; % fine ones!!!!!!!!!!!!!!!!!!!!!!!! CHECK HERE 
        elseif mask(i) == 2 % !!!!!!!!!!!!!!!!!!!!!!!! CHECK HERE 
            for s = 1:sv  % for each scale
               a = floor(a/2);
               b = floor(b/2);
            end
            m = sum([a b].*inda2) + 1;
            cind2(m,k) = m;  % coarse ones !!!!!!!!!!!!!!!!!!!!!!!! CHECK HERE 
        end
    end
    % prunning
    for i = 1:size(mask(:),1)
