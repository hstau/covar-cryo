...
for n = 1:30
    ...
    for i = 1:I % for each view (block-ART)
        ...
        if (size(prj_sel,1) > th)
            ...
            string=strcat('covar/stats/ecovar_',type,num2str(l,'%05d'),'.txt'); % run 
            data = load(string);
    ...
    str = strcat('evar_',num2str(const),'_3a',type,num2str(n,'%02d'),'_',num2str(th,'%03d'),'.spi');
    ...
    grs = grs / cc
    report(n) = resid;
    string =   strcat('ereport_',num2str(const),'_3a',type,num2str(n,'%05d'),'_',num2str(th,'%03d'));
    save(string,'report');
    %array_total = array;
    string = strcat('earray_total_',num2str(const),'_3a',type,num2str(n,'%05d'),'_',num2str(th,'%03d'));
